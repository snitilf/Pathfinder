...
Pathfinder's coupling to Claude Code is a small, enumerable surface: model aliases, settings keys, hook events and payload fields, and subagent frontmatter. Everything else in the repo is role-based policy prose that no Claude Code update can break. Re-run this checklist when Claude Code updates; the optional version-watch hook (`templates/hooks/pathfinder-version-watch.md`) injects a reminder at session start when the version changes.
...
| e | Subagent frontmatter supports `model`, `effort`, `disallowedTools` | Claude Code sub-agents docs | `templates/agents/*.md`, `install/AGENT-INSTALL.md` preflight |
| f | Built-in Explore inherits the main-session model unless overridden (pathfinder pins it to haiku) | model-config / sub-agents docs | `templates/agents/Explore.md`, `docs/design.md` |
